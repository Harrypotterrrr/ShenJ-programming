...
int memory_speed_test(int op)
{
	...
	cout << "          4、因为虚拟机机制，Linux不会出现耗尽内存的错误，但会把宿主机内存耗尽并卡顿，同时虚拟机出现重启/关机等故障" << endl;
	...
	*crc_str = 0; //置为空串
	...
	if (check1 || check2)
		...
		if (op == 0) {
			...
			cout << "    老师的机器运行VS2017编译的程序，大约耗时450-500秒" << endl;
			cout << "    老师的机器运行Linux编译的程序，大约耗时160-180秒" << endl;
		}
		else {
			cout << "内存性能测试(s1+=str方式)" << endl;
			cout << "    老师的机器运行VS2017编译的程序，大约耗时200-240秒" << endl;
			cout << "    老师的机器运行Linux编译的程序，大约耗时100-120秒" << endl;
		}
		cout << "    【说明】：只有相同编译器下的运行时间才有可比性" << endl;
		cout << "              老师的机器为Intel(R) Core(TM) i5-4210U CPU @1.70GHz  2.40GHz（虚拟机为1CPU，2G内存）" << endl;
